...
class WindResourceNodes(WindResource):
    ...
    def get_site_conditions(self, locations, wdirs):
        ...
        for i in range(n):
            ...
            # Select and subset the dataset by the indicies
            # AND squeeze the dimensions that are size=1, making
            # The values scalar instead.
            ds_loc = ds.isel(n=indn, sec=inds, z=indz).squeeze()
            ds_loc.coords['n'] = i + 1
            ...
            if first:
                ...
                ds_out = xr.concat([ds_out, ds_loc], dim='n')
